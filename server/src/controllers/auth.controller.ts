...
export const register = async (req: Request, res: Response) => {
  ...
  await RefreshToken.create({ token: refreshToken, user: user._id, expiresAt: new Date(Date.now() + 7 * 86400000) })
...
export const login = async (req: Request, res: Response) => {
  ...
  // Clean expired tokens, store new one
...
export const refreshTokens = async (req: Request, res: Response) => {
  ...
  if (!stored || stored.expiresAt < new Date()) throw new UnauthorizedError('Token expired')
...
export const logout = async (req: Request, res: Response) => {
  ...
  if (token) await RefreshToken.deleteMany({ token }).catch(() => {})
